...
def underpaid_overpaid(file):
    ...
    for line in file_open:
        ...
        line.rstrip()
        lst = line.split("|")
        customer_name = lst[1].split(" ")[0]
        num_melons = int(lst[2])
        melons_paid = float(lst[3])

        amount_expected = num_melons * melon_cost

        if amount_expected != melons_paid:
            print customer_name, "paid {:.2f}, expected {:.2f}".format(melons_paid, amount_expected)

underpaid_overpaid("customer-orders.txt")          
